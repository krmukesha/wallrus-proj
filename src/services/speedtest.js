...
Speedtest.prototype = {
  ...
  _checkServerDefinition: function (server) {
    try {
      if (typeof server.name !== "string")
        ...
        console.error("Name string missing from server definition (name)");
      ...
      console.error("Invalid server definition");
    }
  },
  /**
  ...
  addTestPoint: function (server) {
    ...
    if (this._state !== 1)
      console.error("You can't add a server after server selection");
  ...
  getSelectedServer: function () {
    if (this._state < 2 || this._selectedServer == null)
      ...
      console.error("No server is selected");
  ...
  setSelectedServer: function (server) {
    ...
    if (this._state === 3)
      ...
      console.error("You can't select a server while the test is running");
  ...
  selectServer: function (result) {
    if (this._state !== 1) {
      ...
      if (this._state === 0) console.error("No test points added");
      // if (this._state === 2) throw "Server already selected";
      if (this._state === 2) console.error("Server already selected");
      if (this._state >= 3)
        // throw "You can't select a server while the test is running";
        console.error("You can't select a server while the test is running");
    }
    if (this._selectServerCalled) console.error("selectServer already called");
     ...
     */
    let select = function (serverList, selected) {
      ...
      let PINGS = 3, //up to 3 pings are performed, unless the server is down...
        SLOW_THRESHOLD = 500; //...or one of the pings is above this threshold
      let checkServer = function (server, done) {
        let i = 0;
        server.pingT = -1;
   ...
   */
  start: function () {
    if (this._state === 3) console.error("Test already running");
  ...
  abort: function () {
    if (this._state < 3)
      console.error("You cannot abort a test that's not started yet");
