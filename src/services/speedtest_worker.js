...
const isSpeedTestEvent = (e) =>
  e.data && Object.prototype.hasOwnProperty.call(e.data, "split");
...
*/
export default function setupSpeedTestSW() {
  window.addEventListener("message", function (e) {
    if (!isSpeedTestEvent(e)) return;
    let runNextTest;
    ...
    if (params[0] === "start" && testState === -1) {
      ...
      try {
        ...
        if (
          ...
          if (/Chrome.(\d+)/i.test(ua) && !!window.fetch) {
            if (typeof s.xhr_dlMultistream === "undefined") {
      ...
      function () {
        ...
        if (!graceTimeDone) {
          ...
          if ((t + bonusT) / 1000.0 > settings.time_ul_max || failed) {
            ...
            ulProgress = 1;
            tlog(
              `ulTest: ${ulStatus}, took ${new Date().getTime() - startT}ms`,
            );
            done();
          }
        }
  ...
  const doPing = function () {
    ...
    xhr[0].onload = function () {
      ...
      if (i < settings.count_ping) doPing();
        ...
        pingProgress = 1;
        tlog(
          `ping: ${pingStatus} jitter: ${jitterStatus}, took ${
...
function sendTelemetry(done) {
  ...
  try {
    ...
    let postData = `extra=${encodeURIComponent(
      settings.telemetry_extra,
    )}&ispinfo=${encodeURIComponent(
      JSON.stringify(telemetryIspInfo),
    )}&dl=${encodeURIComponent(dlStatus)}&ul=${encodeURIComponent(
      ulStatus,
    )}&ping=${encodeURIComponent(pingStatus)}&jitter=${encodeURIComponent(
      jitterStatus,
